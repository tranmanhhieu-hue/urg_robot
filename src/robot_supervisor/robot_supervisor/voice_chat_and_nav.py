...
NAV_ALERT_TOPIC = "/nav_alert_text"
...
EXIT_COMMANDS = ["tạm biệt", "thoát", "bye"]
CANCEL_COMMANDS = ["dừng", "dừng lại", "đứng lại", "hủy", "huỷ", "ngừng lại"]
...
class VoiceRosBridge(Node):
    def __init__(self):
        ...
        self.nav_alert_sub = self.create_subscription(
            String,
            NAV_ALERT_TOPIC,
            self.on_nav_alert,
            10
...
def main():
    ...
    try:
        while True:
            ...
            text_lower = you.lower()

            # Lệnh dừng robot khi đang tự hành
            if has_wake_word(you) and any(cmd in text_lower for cmd in CANCEL_COMMANDS):
                bridge.send_cancel()
                bridge.busy = False
                pending_goal = None
                speak("Tôi đã dừng lại.")
                continue

            # Lệnh thoát chương trình giao tiếp
